 ...
 * @date Dec 31, 2013
 */
public class FaultTolerantSchedulingExample2 extends WorkflowSimBasicExample1 {

    ////////////////////////// STATIC METHODS ///////////////////////
    public static void main(String[] args) {
//        run();
        List<List<Job>> outputList = new ArrayList<>();
        double timeSum = 0;
        List<Double> timeList = new ArrayList<>();
        for(int i = 0; i < 1000; i++){
            List<Job> outputList0 = run();
            outputList.add(outputList0);
            timeSum += outputList0.get(outputList0.size() - 1).getFinishTime();
            timeList.add(outputList0.get(outputList0.size() - 1).getFinishTime());
        }
    ...
    public static List<Job> run() {
        ...
        try {
             ...
             */
            String daxPath = "../WorkflowSim-1.0/config/dax/Montage_100.xml";
             ...
             */
            FailureParameters.FTCFailureCluster failureClusterMode = FailureParameters.FTCFailureCluster.FTCFailureCluster_NONE;

            FailureParameters.FTCluteringAlgorithm ftc_method = FailureParameters.FTCluteringAlgorithm.FTCLUSTERING_NOOP;

            FailureParameters.FTSchedulingAlgorithm ftScheduling_method = FailureParameters.FTSchedulingAlgorithm.FTScheduling_HEURISTIC;
            /**
            ...
            FailureParameters.setSchedulingAlgorithm(ftScheduling_method);
            FailureParameters.setFTCFailureCluster(failureClusterMode);
             ...
             */
            List<CondorVM> vmlist0 = createVM(wfEngine.getSchedulerId(0), Parameters.getVmNum() / 2);
            vmlist0.addAll(createFasterVM(wfEngine.getSchedulerId(0), Parameters.getVmNum() / 2, Parameters.getVmNum() / 2));
