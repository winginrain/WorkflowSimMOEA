...
public class WorkflowSimBasicExample1 {
    ...
    protected static List<CondorVM> createFasterVM(int userId, int vms, int index) {
        //Creates a container to store VMs. This list is passed to the broker later
        LinkedList<CondorVM> list = new LinkedList<>();
        ...
        double costPerStorage = 0.2;	// the cost of using storage in this resource
        double costPerBw = 0.2;			// the cost of using bw in this resource
        //create VMs
        CondorVM[] vm = new CondorVM[vms];
        for (int i = 0; i < vms; i++) {
            double ratio = 1.0;
            vm[i] = new CondorVM(i + index, userId, mips * ratio, pesNumber, ram, bw, size, vmm,
                    cost, costPerMem, costPerStorage, costPerBw, new CloudletSchedulerSpaceShared());
            list.add(vm[i]);
        }
        return list;
