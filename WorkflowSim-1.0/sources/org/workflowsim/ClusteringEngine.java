...
public final class ClusteringEngine extends SimEntity {
    ...
    public final Job addTasks2Job(Task task) {
        return engine.addTasks2Job(task);
    }
    /**
        ...
        switch (ev.getTag()) {
            case WorkflowSimTags.START_SIMULATION:
                if(FailureParameters.getFailureClusterMode() != FailureParameters.FTCFailureCluster.FTCFailureCluster_NONE){
                    send(this.workflowEngineId, FailureParameters.getFailureClusteringDelay(), WorkflowSimTags.CLOUDLET_FAIL_HANDLE, null);
                }
                break;
