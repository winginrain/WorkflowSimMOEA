/**
 * Copyright 2012-2013 University Of Southern California
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not
 * use this file except in compliance with the License. You may obtain a copy of
 * the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
 * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the
 * License for the specific language governing permissions and limitations under
 * the License.
 */
package org.workflowsim.reclustering;

import java.util.*;

import org.cloudbus.cloudsim.Cloudlet;
import org.cloudbus.cloudsim.Log;
import org.cloudbus.cloudsim.Vm;
import org.cloudbus.cloudsim.core.CloudSim;
import org.moeaframework.Executor;
import org.moeaframework.core.NondominatedPopulation;
import org.moeaframework.core.Solution;
import org.moeaframework.core.variable.EncodingUtils;
import org.moeaframework.core.variable.Subset;
import org.moeaframework.problem.workflow.WorkflowSimProblem;
import org.moeaframework.problem.workflow.WorkflowSimRunningProblem;
import org.workflowsim.CondorVM;
import org.workflowsim.Job;
import org.workflowsim.Task;
import org.workflowsim.failure.FailureMonitor;
import org.workflowsim.failure.FailureParameters;
import org.workflowsim.failure.FailureRecord;
import org.workflowsim.utils.Parameters;

/**
 *
 * A ReclusteringEngine creates a new clustered job after a job fails
 *
 * @author Weiwei Chen
 * @since WorkflowSim Toolkit 1.0
 * @date Apr 9, 2013
 *
 */
public class ReclusteringEngine {

    /**
     * the vm list.
     */
    private static List<? extends Vm> vmList;
    
    private static List<Job> failedJobList = new ArrayList<>();

    public static void setVmList(List<? extends Vm> vmList) {
        ReclusteringEngine.vmList = vmList;
    }

    public static void addFailedJob(Job failedJob){
        if (failedJobList == null){
            failedJobList = new ArrayList<>();
        }
        failedJobList.add(failedJob);
    }

    public static List<Job> getFailedJobList() {
        if (failedJobList == null){
            failedJobList = new ArrayList<>();
        }
        return failedJobList;
    }

    public static List<Job> handleFailedJob(int newId){
        if(failedJobList != null && !failedJobList.isEmpty()){
            List<Job> jobList = ReclusteringEngine.process(failedJobList, newId);
            failedJobList.clear();
            return jobList;
        }

        return new ArrayList<>();
    }
    /**
     * Create a new job
     *
     * @param id, the job id
     * @param job, the failed job
     * @param length, the length of this job
     * @param taskList, the task list
     * @return a new job
     */
    private static Job createJob(int id, Job job, long length, List<Task> taskList, boolean updateDep) {
        try {
            Job newJob = new Job(id, length);
            newJob.setUserId(job.getUserId());
            newJob.setVmId(-1);
            newJob.setCloudletStatus(Cloudlet.CREATED);

            newJob.setTaskList(taskList);
            newJob.setDepth(job.getDepth());
            newJob.setExecStartTime(job.getFinishTime());
            if (updateDep) {
                newJob.setChildList(job.getChildList());
                newJob.setParentList(job.getParentList());
                for (Iterator it = job.getChildList().iterator(); it.hasNext();) {
                    Job cJob = (Job) it.next();
                    cJob.addParent(newJob);
                }
            }
            return newJob;
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;

    }

    /**
     * Process job recreation based on different reclustering algorithm
     *
     * @param failedJobListJob, job
     * @param id, job id
     * @return a list of new jobs
     */
    public static List<Job> process(List<Job> failedJobListJob, int id) {
        List<Job> jobList = new ArrayList();
        List<Task> allTaskList = new ArrayList<>();
        failedJobListJob.forEach(j -> allTaskList.addAll(j.getTaskList()));
        try {
            switch (FailureParameters.getFTCluteringAlgorithm()) {
                case FTCLUSTERING_NOOP:
                    for (Job job: failedJobListJob){
                        jobList.add(createJob(id, job, job.getCloudletLength(), job.getTaskList(), true));
                        id++;
                    }
                    //job submttted doesn't have to be considered
                    break;
                /**
                 * Dynamic clustering.
                 */
                case FTCLUSTERING_DC:

                    jobList = DCReclustering(jobList, failedJobListJob, id, allTaskList);
                    break;
                /**
                 * Selective reclustering.
                 */
                case FTCLUSTERING_SR:
                    jobList = SRReclustering(jobList, failedJobListJob, id);

                    break;
                /**
                 * Dynamic reclustering.
                 */
                case FTCLUSTERING_DR:
                    jobList = DRReclustering(jobList, failedJobListJob, id, allTaskList);
                    break;
                /**
                 * Block reclustering.
                 */
                case FTCLUSTERING_BLOCK:
                    jobList = BlockReclustering(jobList, failedJobListJob, id);
                    break;
                /**
                 * Binary reclustering.
                 */
                case FTCLUSTERING_VERTICAL:
                    jobList = VerticalReclustering(jobList, failedJobListJob, id);
                    break;
                default:
                    break;
            }

            switch (FailureParameters.getFTSchedulingAlgorithm()){
                case FTScheduling_STATIC:
                    StaticRescheduling(jobList);
                    break;
                case FTScheduling_RANDOM:
                    RandomRescheduling(jobList);
                    break;
                case FTScheduling_NSGA3:
                    NSGARescheduling(jobList);


                    break;
                case FTScheduling_HEURISTIC:
                    HeuristicRescheduling(jobList);
                    break;
                default:
                    break;
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return jobList;
    }

    private static void ScheduleSimpleJob(Job job, List<Job> allocatedJobList) {
        List<Job> jobList = new ArrayList<>();
        jobList.add(job);
        WorkflowSimRunningProblem workflowSimProblem = new WorkflowSimRunningProblem(jobList, allocatedJobList, vmList, 2);
        List<Solution> res = new ArrayList<>();
        for(int index = 0; index < jobList.size(); index++){
            for(int vmId = 0; vmId < vmList.size(); vmId++){
                Solution solution = workflowSimProblem.newSolution();
                Subset s = (Subset) solution.getVariable(index);
                s.clear();
                s.add(vmId);
                workflowSimProblem.evaluate(solution);
                res.add(solution);
            }
        }

        Solution minSolution = res.get(0);
        for(Solution solution: res){
            if(solution.getObjective(0) < minSolution.getObjective(0)){
                minSolution = solution;
            }
        }

        for(Job j : jobList){
            int vmId = EncodingUtils.getSubset(minSolution.getVariable(0))[0];
            j.setVmId(vmId);
            workflowSimProblem.evaluate(minSolution);
        }
    }

    private static void HeuristicRescheduling(List<Job> jobList) {
//        for (Job job : jobList){
//            List<Task> spouseList = job.getSpouseList();
//            for(Task t: spouseList){
//
//            }
//        }
//        for(Vm vm: vmList){
//            vm.getCloudletScheduler().getCloudletSubmittedList();
//        }


        List<Job> allocatedJobList = new ArrayList<>();
        for(Job job: jobList){
            ScheduleSimpleJob(job, allocatedJobList);
            allocatedJobList.add(job);
        }
    }

    private static void StaticRescheduling(List<Job> jobList) {
        for(Job j : jobList){
            j.setVmId(j.getTaskList().get(0).getVmId());
        }
    }

    private static void RandomRescheduling(List<Job> jobList) {
        for(Job j : jobList){
            Random random = new Random();
            int vmId = random.nextInt(vmList.size());
            j.setVmId(vmId);
        }
    }

    private static void NSGARescheduling(List<Job> jobList) {
        WorkflowSimRunningProblem workflowSimProblem = new WorkflowSimRunningProblem(jobList,  vmList, 2);
        NondominatedPopulation result = new Executor()
                .withProblem(workflowSimProblem)
                .withAlgorithm("NSGAIII")
                .withMaxEvaluations(10000)
                .run();
        Solution solution = result.get(0);
        workflowSimProblem.evaluate(solution);
        for (Solution s : result){
            if(s.getObjective(0) < solution.getObjective(0)){
                solution = s;
            }
        }
        for(int i = 0; i < jobList.size(); i++){
            Job job = jobList.get(i);
            int vmIndex = EncodingUtils.getSubset(solution.getVariable(i))[0];
            job.setVmId(vmIndex);
        }
    }

    /**
     * Partition the list of tasks based on their depth
     *
     * @param list, the list to process
     * @return a map with key equals to depth
     */
    private static Map<Integer, List<Task>> getDepthMap(List<Task> list) {
        Map<Integer, List<Task>> map = new HashMap<>();

        for (Task task : list) {
            int depth = task.getDepth();
            if (!map.containsKey(depth)) {
                map.put(depth, new ArrayList<>());
            }
            List<Task> dl = map.get(depth);
            if (!dl.contains(task)) {
                dl.add(task);
            }
        }

        return map;
    }
    //DR + level aware

    /**
     * Used in DR and level aware
     *
     * @param map
     * @return the minimum key in Map
     */
    private static int getMin(Map<Integer, List<Task>> map) {
        if (map != null && !map.isEmpty()) {
            int min = Integer.MAX_VALUE;
            for (int value : map.keySet()) {
                if (value < min) {
                    min = value;
                }
            }
            return min;
        }
        return -1;
    }

    /**
     * Check whether this list has failed task
     *
     * @param list
     * @return boolean whether this list has failed task
     */
    private static boolean checkFailed(List<Task> list) {
        boolean all = false;
        for (Task task : list) {
            if (task.getCloudletStatus() == Cloudlet.FAILED) {
                all = true;
                break;
            }
        }
        return all;
    }

    /**
     * Vertical Reclustering
     *
     * @param jobList, job list
     * @param failedJobListJob, job
     * @param id, job id
     * @return
     */
    private static List<Job> VerticalReclustering(List<Job> jobList, List<Job> failedJobListJob, int id) {
        Job job = failedJobListJob.get(0);
        Map<Integer, List<Task>> map = getDepthMap(job.getTaskList());
        
        /**
         * If it has just one level
         */
        if (map.size() == 1) {

            jobList = DCReclustering(jobList, failedJobListJob, id, job.getTaskList());

            return jobList;
        }
        int min = getMin(map);
        int max = min + map.size() - 1;
        int mid = (min + max) / 2;
        List listUp = new ArrayList<>();
        List listDown = new ArrayList<>();
        for (int i = min; i < min + map.size(); i++) {
            List<Task> list = map.get(i);
            if (i <= mid) {
                listUp.addAll(list);
            } else {
                listDown.addAll(list);
            }
        }
        List<Job> newUpList = DCReclustering(new ArrayList<>(), failedJobListJob, id, listUp);
        id += newUpList.size();
        jobList.addAll(newUpList);
        jobList.addAll(DCReclustering(new ArrayList<>(), failedJobListJob, id, listDown));
        return jobList;

    }

    /**
     * Block Reclustering
     *
     * @param jobList, job list
     * @param failedJobList, job
     * @param id, job id
     * @return
     */
    private static List BlockReclustering(List jobList, List<Job> failedJobList, int id) {
        Job job = failedJobList.get(0);
        Map<Integer, List<Task>> map = getDepthMap(job.getTaskList());
        if (map.size() == 1) {
            jobList = DRReclustering(jobList, failedJobList, id, job.getTaskList());
            return jobList;
        }
        //do that to every list
        int min = getMin(map);
        for (int i = min; i < min + map.size(); i++) {

            List<Task> list = map.get(i);

            if (checkFailed(list)) {
                //should be separate
                List newList = DRReclustering(new ArrayList<>(), failedJobList, id, list);
                id = newList.size() + id;//?
                jobList.addAll(newList);
            } else {
                //do nothing
            }
        }


        return jobList;
    }

    /**
     * Dynamic Reclustering
     *
     * @param jobList, job list
     * @param failedJobList, job
     * @param id, job id
     * @param allTaskList, all the task List
     * @return
     */
    private static List<Job> DCReclustering(List<Job> jobList, List<Job> failedJobList, int id, List<Task> allTaskList) {

        Task firstTask = allTaskList.get(0);
        //Definition of FailureRecord(long length, int tasks, int depth, int all, int vm, int job, int workflow)
        /**
         * @TODO do not know why it is here
         */
        Job job = failedJobList.get(0);
        double taskLength = (double) firstTask.getCloudletLength() / 1000 + Parameters.getOverheadParams().getClustDelay(job) / getDividend(job.getDepth());
        FailureRecord record = new FailureRecord(taskLength, 0, job.getDepth(), allTaskList.size(), 0, 0, job.getUserId());
        record.delayLength = getCumulativeDelay(job.getDepth());
        int suggestedK = FailureMonitor.getClusteringFactor(record);

        if (suggestedK == 0) {
            //not really k=0, just too big
            jobList.add(createJob(id, job, job.getCloudletLength(), allTaskList, true));
        } else {

            int actualK = 0;
            List<Task> taskList = new ArrayList<>();
            List<Job> retryJobs = new ArrayList<>();
            long length = 0;
            Job newJob = createJob(id, job, 0, null, false);
            for (Task allTaskList1 : allTaskList) {
                Task task = (Task) allTaskList1;
                if (actualK < suggestedK) {
                    actualK++;
                    taskList.add(task);
                    length += task.getCloudletLength();
                } else {
                    newJob.setTaskList(taskList);
                    taskList = new ArrayList<>();
                    newJob.setCloudletLength(length);
                    length = 0;
                    retryJobs.add(newJob);
                    id++;
                    newJob = createJob(id, job, 0, null, false);
                    actualK = 0;//really a f*k bug
                }
            }
            
            if (!taskList.isEmpty()) {
                newJob.setTaskList(taskList);
                newJob.setCloudletLength(length);
                retryJobs.add(newJob);
            }
            updateDependencies (job, retryJobs);
            jobList.addAll(retryJobs);

        }
        return jobList;

    }

    /**
     * Selective Reclustering
     *
     * @param jobList, job list
     * @param failedJobListJob, job
     * @param id, job id
     * @return
     */
    private static List<Job> SRReclustering(List<Job> jobList, List<Job> failedJobListJob, int id) {
        List<Task> newTaskList = new ArrayList<>();
        Job job = failedJobListJob.get(0);
        long length = 0;
        for (Task task : job.getTaskList()) {
            if (task.getCloudletStatus() == Cloudlet.FAILED) {
                newTaskList.add(task);
                length += task.getCloudletLength();
            } else {
            }
        }
        jobList.add(createJob(id, job, length, newTaskList, true));
        return jobList;
    }

    /**
     * Get the dividend
     *
     * @param depth
     * @return
     */
    private static int getDividend(int depth) {
        int dividend = 1;
        switch (depth) {
            case 1:
                dividend = 78;
                break;
            case 2:
                dividend = 229;
                break;
            case 5:
                dividend = 64;
                break;
            default:
                Log.printLine("Eroor");
                break;
        }
        return dividend;
    }

    /**
     * Sum up all the delay for one job
     * @param depth the depth
     * @return cumulative delay
     */
    private static double getCumulativeDelay(int depth){
        double delay = 0.0;
        if(Parameters.getOverheadParams().getQueueDelay()!=null && 
                Parameters.getOverheadParams().getQueueDelay().containsKey(depth)){
            delay += Parameters.getOverheadParams().getQueueDelay().get(depth).getMLEMean();
        }
        if(Parameters.getOverheadParams().getWEDDelay()!=null &&
                Parameters.getOverheadParams().getWEDDelay().containsKey(depth)){
            delay += Parameters.getOverheadParams().getWEDDelay().get(depth).getMLEMean();
        }
        if(Parameters.getOverheadParams().getPostDelay()!=null &&
                Parameters.getOverheadParams().getPostDelay().containsKey(depth)){
            delay += Parameters.getOverheadParams().getPostDelay().get(depth).getMLEMean();
        }
        return delay;
    }
    
    private static double getOverheadLikelihoodPrior(int depth){
        double prior = 0.0;

        if(Parameters.getOverheadParams().getQueueDelay()!=null && 
                Parameters.getOverheadParams().getQueueDelay().containsKey(depth)){
            prior = Parameters.getOverheadParams().getQueueDelay().get(depth).getLikelihoodPrior();
        }else
        if(Parameters.getOverheadParams().getWEDDelay()!=null &&
                Parameters.getOverheadParams().getWEDDelay().containsKey(depth)){
            prior = Parameters.getOverheadParams().getWEDDelay().get(depth).getMLEMean();
        }else
        if(Parameters.getOverheadParams().getPostDelay()!=null &&
                Parameters.getOverheadParams().getPostDelay().containsKey(depth)){
            prior = Parameters.getOverheadParams().getPostDelay().get(depth).getMLEMean();
        }
        return prior;
    }
    
    /**
     * Dynamic Reclustering
     *
     * @param jobList, job list
     * @param failedJobListJob, job
     * @param id, job id
     * @param allTaskList, all task list
     * @return
     */
    private static List DRReclustering(List<Job> jobList, List<Job> failedJobListJob, int id, List<Task> allTaskList) {
        Task firstTask = allTaskList.get(0);
        Job job = failedJobListJob.get(0);
        /**
         * Do not know why it is here. It is hard-coded, right?
         */
        double taskLength = (double) firstTask.getCloudletLength() / 1000 ;//+ Parameters.getOverheadParams().getClustDelay(job) / getDividend(job.getDepth());
        
        //FailureRecord record = new FailureRecord(taskLength, 0, job.getDepth(), allTaskList.size(), 0, 0, job.getUserId());
        
        double phi_ts = getOverheadLikelihoodPrior(job.getDepth());
        double delay = getCumulativeDelay(job.getDepth());
        //record.delayLength = getCumulativeDelay(job.getDepth());
        int suggestedK;//FailureMonitor.getClusteringFactor(record);
        
        double theta = FailureParameters.getGenerator(job.getVmId(), job.getDepth()).getMLEMean();
        
        double phi_gamma = FailureParameters.getGenerator(job.getVmId(), job.getDepth()).getLikelihoodPrior();
        suggestedK = ClusteringSizeEstimator.estimateK(taskLength, delay, 
                theta, phi_gamma, phi_ts);

        Log.printLine("t=" + taskLength +" d=" + delay + " theta=" + theta + " k=" + suggestedK);
        if (suggestedK == 0) {
            //not really k=0, just too big
            jobList.add(createJob(id, job, job.getCloudletLength(), allTaskList, true));
        } else {

            int actualK = 0;
            List<Task> taskList = new ArrayList<>();
            List<Job> retryJobs = new ArrayList<>();
            long length = 0;
            Job newJob = createJob(id, job, 0, null, false);
            for (Task task : allTaskList) {
                if (task.getCloudletStatus() == Cloudlet.FAILED) {//This is the difference
                    if (actualK < suggestedK) {
                        actualK++;
                        taskList.add(task);
                        length += task.getCloudletLength();
                    } else {
                        newJob.setTaskList(taskList);
                        taskList = new ArrayList<>();
                        newJob.setCloudletLength(length);
                        length = 0;
                        retryJobs.add(newJob);
                        id++;
                        newJob = createJob(id, job, 0, null, false);
                        actualK = 0;
                    }
                }
            }

            if (!taskList.isEmpty()) {
                newJob.setTaskList(taskList);
                newJob.setCloudletLength(length);
                retryJobs.add(newJob);
            }
            updateDependencies (job, retryJobs);
            jobList.addAll(retryJobs);

        }
        return jobList;
    }
    
    private static void updateDependencies (Job job, List<Job> jobList) {
        // Key idea: avoid setChildList(job.getChildList) within the for loop since
        // it will override the list
        List<Task> parents = job.getParentList();
        List<Task> children = job.getChildList();
        for (Job rawJob : jobList) { 
            rawJob.setChildList(children);
            rawJob.setParentList(parents);
        }
        for (Task parent : parents) {
            parent.getChildList().addAll(jobList);
        }
        for (Task childTask : children) {
            Job childJob = (Job) childTask;
            childJob.getParentList().addAll(jobList);
        }
    }
}
