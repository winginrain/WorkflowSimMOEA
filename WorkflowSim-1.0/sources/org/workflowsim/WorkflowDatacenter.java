...
public class WorkflowDatacenter extends Datacenter {
    ...
    public double getEstimatedFinishTime(Task job, CondorVM vm, double startTime){
        /**
         * Add data transfer time (communication cost
         */
        double fileTransferTime = getFileTransferTime(job.getFileList(), job);
        CloudletScheduler scheduler = vm.getCloudletScheduler();
        double estimatedFinishTime = scheduler.getCloudletCompleteTime(job, vm, startTime, fileTransferTime);
        return estimatedFinishTime;
    }

    public void resetDelayTime(CondorVM vm){
        CloudletScheduler scheduler = vm.getCloudletScheduler();
        scheduler.setDelayTime(0f);
    }
    ...
    protected double getFileTransferTime(List<FileItem> requiredFiles, Task job) {
        double time = 0.0;
