...
public class HEFTPlanningAlgorithm extends BasePlanningAlgorithm {
    ...
    public void initialHEFTSolution(Solution s){
        ...
        for (TaskRank rank : taskRank) {
            allocateTask(rank.task, s);
        }

    }

    private void allocateTask(Task task, Solution s) {
        CondorVM chosenVM = null;
        double earliestFinishTime = Double.MAX_VALUE;
        double bestReadyTime = 0.0;
        double finishTime;
        int[] randArray = CommonUtils.getRandomArray(getVmList().size());
        for (int i = 0; i< getVmList().size(); i++) {
            CondorVM vm = (CondorVM) getVmList().get(randArray[i]);
        ...
        Subset subset = (Subset)s.getVariable(task.getCloudletId() - 1);
        subset.clear();
        subset.add(chosenVM.getId());
    }
